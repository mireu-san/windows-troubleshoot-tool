...
func handleAppUpdate() bool {
	...
	if err := applyAppUpdate(target, staged, pidText, digest); err != nil {
		...
		title, _ := windows.UTF16PtrFromString(nativeText("앱 업데이트 실패"))
		message, _ := windows.UTF16PtrFromString(nativeText("업데이트를 적용하지 못했습니다. 기존 앱을 다시 실행해 주세요.\n\n") + nativeText(err.Error()))
