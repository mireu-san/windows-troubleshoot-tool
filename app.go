...
func NewApp() *App {
	return &App{runner: newSystemRunner()}
}
...
func (a *App) beforeClose(ctx context.Context) bool {
	...
	const waitButton = "계속 기다리기"
	const closeButton = "지금 종료"
	selection, err := runtime.MessageDialog(ctx, runtime.MessageDialogOptions{
		Type:          runtime.WarningDialog,
		Title:         "검사 또는 복구 작업이 진행 중입니다",
		Message:       "급히 나가야 한다면 지금 종료할 수 있습니다. 진행 중인 작업은 완료되지 않았을 수 있으며, 이미 적용된 변경은 되돌리지 않습니다.\n\n시스템 검사 및 복구는 취소 버튼으로 중단을 확인한 뒤 닫는 것을 권장합니다. 업데이트 캐시 초기화 중이라면 서비스 복원이 끝날 때까지 기다려 주세요.\n\n다음에 앱을 열어 다시 검사할 수 있습니다.",
