    ...
    <details class="advanced-panel">
      ...
      <div class="advanced-content">
        ...
        <div class="update-progress" id="updateProgress" hidden>
          ...
          <p class="hint">명령 출력은 Windows에서 제공한 원문입니다.</p><pre id="updateOutput" aria-live="polite"></pre>
...
cancelRepairButton.addEventListener('click', async () => {
  if (!window.confirm(t('진행 중인 복구 명령을 중단합니다. 이미 적용된 변경은 되돌리지 않으며, 재시작이 필요할 수 있습니다. 취소하시겠습니까?'))) return;
...
els.start.addEventListener('click', async () => {
  ...
  if (shutdown && !window.confirm(t('검사와 복구가 정상적으로 끝나면 5분 뒤 컴퓨터가 꺼집니다. 저장하지 않은 작업 내용은 사라질 수 있으니 먼저 저장해 주세요. 완료 후 컴퓨터를 끌까요?'))) return;
...
els.quickAssist.addEventListener('click', async () => {
  ...
  try {
    ...
    window.alert(t('빠른 지원을 열 수 없습니다. Microsoft Store에서 빠른 지원 앱이 설치되어 있는지 확인해 주세요.') + '\n\n' + t(String(error)));
  } finally {
...
els.updateTroubleshooter.addEventListener('click', async () => {
  ...
  try {
    ...
    window.alert(t('Windows 업데이트 문제 해결사를 열 수 없습니다.') + '\n\n' + t(String(error)));
  }
});
