...
app.innerHTML = `
  <main class="shell">
    ...
    <section class="action-panel">
      <div class="shutdown-options">
        <label><input type="checkbox" id="shutdownAfterRepair"> DISM과 SFC 모두 성공하면 5분 뒤 컴퓨터 종료</label>
        <p class="hint">종료 예약 시 열려 있는 앱이 강제로 닫힐 수 있습니다. 파일을 먼저 저장하세요. 앱을 닫아도 예약은 유지됩니다.</p>
  ...
  <dialog class="tool-overview developer-info" id="developerInfo" aria-labelledby="developerTitle">
    <form method="dialog">
      ...
      <p class="developer-purpose">비상업적 목적으로 개발했습니다.</p>
      <p>MIT License · © 2026 Ayin Kim</p>
      <p class="developer-license-note">라이선스에 따라 상업적 이용·수정·재배포가 가능합니다.</p>
...
els.start.addEventListener('click', async () => {
  ...
  if (shutdown && !window.confirm('DISM과 SFC가 모두 성공하면 5분 뒤 컴퓨터가 종료됩니다. 열려 있는 앱이 강제로 닫혀 저장하지 않은 내용이 손실될 수 있습니다. 종료 예약을 사용하시겠습니까?')) return;
